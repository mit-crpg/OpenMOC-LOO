    ...
    if (cell->getType() == MATERIAL) {
        ...
        if (t_num_rings > 1) 
        {
            ...
            if (cell->getNumSurfaces() == 1) 
            {
                ...
                /* update the original circle to be innermost cell */
                static_cast<Cell*>(cell)->removeSurface(-surface_id);
            ...
            else if (cell->getNumSurfaces() == 2) 
            {
                ...
                /* update the innermost cell: keep -old_id, add inner_surface */
                static_cast<Cell*>(cell)->removeSurface(-outer_surface);
                static_cast<Cell*>(cell)->addSurface(-old_id, s);
                id++;
                ...
                int tmp[2] = {old_id, -1 * outer_surface};
                int *list_surfaces = &tmp[0];
                c = new CellBasic
                    (id, cell->getUniverse(), 2, list_surfaces,
                     dynamic_cast<CellBasic*>(cell)->getMaterial(), 0,
                          ...
                          int* meshCellNum, int row, bool base, int fsr_id)
{
    ...
    if (univ->getType() == SIMPLE)
    {
        ...
        for (iter = cells.begin(); iter != cells.end(); ++iter) 
        {
            curr = iter->second;CellFill* fill_cell = 
                                    static_cast<CellFill*>(curr);
            Universe* universe_fill = fill_cell->getUniverseFill();
            defineMesh(mesh, universe_fill, depth, meshCellNum, row, base, 
                       fsr_id);
        }
    }
    /* If the universe is a LATTICE type universe */
            ...
            for (int i = num_y-1; i > -1; i--) 
            {
                ...
                for (int k = nextHeight-1; k > -1; k--) 
                {
                    for (int j = 0; j < num_x; j++) 
                    {
                        ...
                        /* recursively call defineMesh until LATTICE
                         * level of CMFD mesh is reached */
                        defineMesh(mesh, curr, depth - 1, meshCellNum, k, 
                                   base, fsr_id);
                    }
                }
            }
